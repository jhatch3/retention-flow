"""Alembic migration environment for the RetentionFlow database.

The database URL is not stored in ``alembic.ini``; it is read at runtime from
the same :class:`DatabaseSettings` the application uses, so migrations and the
...
from backend.db.config import get_settings  # noqa: E402
