...
The DistilBERT classifier and the LLM judge both emit a single overall quality
grade (1-5), so the four per-dimension columns are removed. ``eval_scores`` now
carries only ``overall_score`` + ``passed``. See backend.eval.distilbert.
